...
from utils.jwt_handler import get_current_user  # JWT 인증된 유저
...
# 📁 유저의 폴더 목록 조회
@router.get("/folders")
def get_user_folders(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def add_recipe_to_folder(folder_id: int, data: FolderRecipeAdd, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def get_folder_recipes(folder_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    folder = db.query(BookmarkFolder).filter_by(id=folder_id, user_id=user.user_id).first()
    if not folder:
        raise HTTPException(status_code=404, detail="❌ 폴더를 찾을 수 없습니다.")
...
def remove_recipe_from_folder(folder_id: int, recipe_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    folder = db.query(BookmarkFolder).filter_by(id=folder_id, user_id=user.user_id).first()
    if not folder:
        raise HTTPException(status_code=404, detail="❌ 폴더를 찾을 수 없습니다.")
